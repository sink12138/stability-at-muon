...
from src.optimizers.muon import muon_update_new
...
class MuonL2PGDAttack(Attack):
    ...
    def forward(self, images, labels):
        ...
        if self.momentum_buffer is None or self.momentum_buffer.size(0) != batch_size: #初始化动量buffer
            self.momentum_buffer = torch.zeros_like(images)
...
class MuonL2PGDAttack_new(Attack):
    def __init__(self, model, eps=1.0, alpha=0.2, steps=10, random_start=True, 
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            if self.momentum_beta > 0 and self.nesterov:
                # 超前位置 = 当前对抗样本 + 动量*beta
                lookahead = adv_images + self.momentum_buffer * self.momentum_beta
                lookahead = torch.clamp(lookahead, min=0, max=1).detach()
                lookahead.requires_grad = True
                outputs = self.get_logits(lookahead)
            else:
                adv_images.requires_grad = True
                outputs = self.get_logits(adv_images)
            ...
            if self.momentum_beta > 0 and self.nesterov:
                grad = torch.autograd.grad(cost, lookahead, retain_graph=False, create_graph=False)[0]
            else:
            ...
            update = muon_update_new(
                ...
                beta=self.momentum_beta,
                ns_steps=self.ns_steps,
                nesterov=self.nesterov  # 启用Nesterov
            ).reshape_as(grad)

            grad_norms = torch.norm(update.view(batch_size, -1), p=2, dim=1) + self.eps_for_division
            update = update / grad_norms.view(batch_size, 1, 1, 1)
